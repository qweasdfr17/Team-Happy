...
describe("WizardStep1Basics", () => {
  ...
  it("emits onChange with ad content mode", () => {
    ...
    fireEvent.click(screen.getByText(/短片\/漫剧片段|Short \/ Manga Clip/));
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ contentMode: "ad" }),
    );
  });

  it("switching to ad defaults generation mode to reference_video", () => {
    const onChange = vi.fn();
    render(
      <WizardStep1Basics
    ...
    fireEvent.click(screen.getByText(/短片\/漫剧片段|Short \/ Manga Clip/));
    expect(onChange).toHaveBeenCalledWith(
      expect.objectContaining({ contentMode: "ad", generationMode: "reference_video" }),
    );
  });

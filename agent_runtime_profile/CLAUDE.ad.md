...
### 视频规格
- **视频比例**：由项目 `aspect_ratio` 配置决定（短片/漫剧片段默认 9:16 竖屏），无需在 prompt 中指定
- **单镜头时长**：短片/漫剧片段项目**没有** `default_duration` 偏好——镜头时长按项目 `target_duration`（目标总时长，秒）逐镜头规划
...
本项目为**短片/漫剧片段模式**（ad），产出**单个**约 `target_duration` 秒的短视频，而非多集系列。产品/带货是可选分支：`products` 为空时自动走通用短片流程：
...
短片/漫剧片段模式仅开放两种**生成模式**（`generation_mode`）：
...
`grid`（宫格生视频）对短片/漫剧片段项目**不开放**：宫格单格分辨率与产品高保真目标冲突。

...
当同一具名角色在剧本中出现明显不同的人生阶段或装束状态时，不要创建独立角色，而应在 Step 5 摘要中标注建议创建 **variants**：

### 触发条件（满足任一即标注）
- 年龄/时期差异：少年 XXX / 成年 XXX / 老年 XXX
- 装束状态差异：战损装 / 便服 / 朝服 / 夜行装
- 剧本中明确描述了同一角色的不同外貌阶段

### 处理方式
1. **基础角色按正常流程写入** characters bucket（如"萧近宸"）
2. **自动创建变体**：在 Step 4 写入基础角色后，立即对每个识别到的变体调用：
   ```
   mcp__arcreel__create_character_variant({
     "character_name": "萧近宸",
     "label": "少年时期",
     "description": "十五六岁，脸部稚嫩，身形清瘦"
   })
   ```
   - 工具自动防重复：同 label 不会重复创建
   - 同角色有多个变体时逐个调用
   - **不要**为每个时期创建独立角色（如"少年萧近宸"作为独立 character 名）——这会破坏角色一致性
   - 创建后变体 design sheet 为空，提示用户后续在 WebUI 上传或生成

3. **服装参考**：如果角色有明确的换装需求且服装差异主要是衣着而非体型/年龄变化：
   - 在摘要中建议创建 costume_references（当前需通过 WebUI 操作）
   - 如果造型变化极大（如战损装、婚服），同时创建 variant

4. **在 Step 5 摘要中列出已创建的变体**：
...
### 服装参考识别
- 如果角色有明确的服装描述且可能换装（如"王爷常服"、"夜行衣"、"朝服"），建议创建 **costume_references**
- 同样在摘要末尾追加「建议服装」表，供用户在 WebUI 中添加

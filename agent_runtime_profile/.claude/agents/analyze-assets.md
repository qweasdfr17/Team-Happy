...
| 角色 | 变体 label | 时期/状态描述 |
|------|-----------|-------------|
| 萧近宸 | 少年时期 | 十五六岁，脸部稚嫩，身形清瘦 |
| 萧近宸 | 成年时期 | 二十五岁左右，气场成熟冷峻 |
| 萧近宸 | 战损装 | 铠甲破损，面部带血痕 |
```

3. **告知主 agent**：variants 需通过 WebUI 角色卡片创建（`mcp__arcreel__patch_project` 不能写入 variants / costume_references），或提示用户手动在角色卡片中添加变体。
4. **不要**为每个时期创建独立角色（如"少年萧近宸"作为独立 character 名）——这会破坏角色一致性。

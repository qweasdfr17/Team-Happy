...
- 视频生成是**高成本操作**，所有 `generate_video_*` 工具需要 `confirmed=true` 才入队。
- 未获得用户明确确认前，**禁止**传 `confirmed=true`。
- "生成提示词""写视频提示词""准备生视频""下一步"都**不等于**确认生成视频。
- 默认只做预检、提示词写入、资产检查。
- 用户必须明确说：「确认生成视频」「开始生成视频」「我同意生成」「执行生视频」才算确认。

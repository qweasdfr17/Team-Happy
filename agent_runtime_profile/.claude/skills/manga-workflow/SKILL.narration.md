...
当用户说"从头重新规划 / 重新规划全部 / 清空分集重新来 / 不要旧分集 / 重新切分全集"时：
- 先用中文确认："这会清空当前分集规划、草稿和已生成剧本派生文件，但保留原始小说和资产库。确认要从头重新规划吗？"
- 用户确认后依次调用：
  1. `mcp__arcreel__reset_episode_plan({"confirm": true, "delete_generated_files": true})`
  2. `mcp__arcreel__plan_episodes({})`
- **禁止**用 `replan_episodes` 处理这种需求——它会从已有分集调整而非清空。

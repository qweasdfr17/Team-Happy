...
@pytest.mark.asyncio
async def test_generate_splits_reference_video_unit_over_15_seconds(tmp_path: Path):
    """LLM 原始输出 17s unit 时，落盘前必须拆成两个 <=15s 的独立 unit。"""
    import json as _j
        ...
        _j.dumps(
            {
                ...
                "content_mode": "narration",
                "generation_mode": "reference_video",
                "_supported_durations": list(range(4, 16)),
                "overview": {"synopsis": "s", "genre": "g", "theme": "t", "world_setting": "w"},
                "style": "国漫",
                "style_description": "水墨",
                "characters": {"主角": {"description": "d"}},
    ...
    fake_generator.generate = AsyncMock(
        return_value=MagicMock(
            text=(
                '{"episode":1,"title":"t",'
                '"video_units":[{"unit_id":"E1U1",'
                '"shots":[{"duration":10,"text":"主角推门进入酒馆"},'
                '{"duration":7,"text":"主角与众人对峙"}],'
                '"references":[{"type":"character","name":"主角"},{"type":"scene","name":"酒馆"}],'
    ...
    units = data["video_units"]
    assert [u["unit_id"] for u in units] == ["E1U1", "E1U2"]
    assert [u["duration_seconds"] for u in units] == [10, 7]
    assert all(u["duration_seconds"] <= 15 for u in units)
    assert all(u["video_prompt_source"] == "skill" for u in units)
    assert "主角推门进入酒馆" in units[0]["shots"][0]["text"]
    assert "主角与众人对峙" in units[1]["shots"][0]["text"]
    assert all("Shot 1" not in u["shots"][0]["text"] for u in units)
    assert all("generated_assets" in u for u in units)
    assert all(u["transition_to_next"] == "cut" for u in units)


@pytest.mark.asyncio
async def test_generate_does_not_trigger_video_generation(tmp_path: Path):
    """ScriptGenerator.generate() 只落剧本和提示词，不触发视频生成任务 / 不入队。"""
    import json as _j
    ...
    fake_generator.generate = AsyncMock(
        return_value=MagicMock(
            text=(
                '{"episode":1,"title":"t",'
                '"video_units":[{"unit_id":"E1U1",'
                '"shots":[{"duration":8,"text":"主角推门进入酒馆"}],'
                '"references":[{"type":"character","name":"主角"},{"type":"scene","name":"酒馆"}],'
    ...
    assert unit["generated_assets"].get("video_clip") is None

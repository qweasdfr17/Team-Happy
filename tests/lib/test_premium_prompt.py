...
class TestApplyPremiumPrompt:
    ...
    def test_chinese_asset_spacing_is_cleaned_in_picture_content(self):
        unit = _multi_shot_unit()
        unit["shots"][0]["text"] = "中学走廊 白天，少年欧阳韬 怀抱 习题册 缓步穿行。左胸 胸针 随步伐微晃。"

        apply_premium_prompt_to_unit(unit, _project())

        text = unit["shots"][0]["text"]
        assert "少年欧阳韬 怀抱" not in text
        assert "习题册 缓步" not in text
        assert "胸针 随" not in text
        assert "中学走廊白天，少年欧阳韬怀抱习题册缓步穿行。左胸胸针随步伐微晃。" in text

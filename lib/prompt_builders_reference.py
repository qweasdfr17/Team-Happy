...
b. **shots**：1-3 个 Shot，每个 shot 是一个视频分镜。
    - 通读 step1_units 中该 unit 的内容，识别其中的场景或空间连续段。
    - **一个 shot / 分镜只能表达一个明确场景或连续动作段，不允许一个 shot 跨多个场景。**
    - 每个场景拆 1-3 个 8-15 秒分镜。
    - `duration`：整数秒（8-15）。分配规则：先算台词时间（短句~1s、中句~2-3s、长句~3-4s），再加动作/反应约 3-5s，再加留白约 1-2s。写完后台词自读一遍，读不完就加时长。{max_duration_line}
...
- 每 unit 最多 3 个 shot；单 shot 8-15 秒；shot 时长之和贴近 step1 预估。
